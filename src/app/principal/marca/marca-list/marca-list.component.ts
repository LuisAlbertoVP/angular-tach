...
export class MarcaListComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    sharedService.buildMenuBar({ title: 'Marcas', addEvent: () => this.openDialog() });
  }
  ...
  updateEstado(marca: Base) {
    ...
    cloneMarca.estado = cloneMarca.estado == 1 ? 0 : 1;
  ...
  delete(id: string) {
    this.service.delete(id).subscribe((response: HttpResponse<string>) => {
      if(response?.status == 200) {
        this.data = this.data.filter(oldMarca => oldMarca.id != id);
        this.showMessage(response.body);
      }
    });
  }
